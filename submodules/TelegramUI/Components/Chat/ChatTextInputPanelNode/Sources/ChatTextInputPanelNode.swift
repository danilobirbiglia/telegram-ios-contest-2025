...
public class ChatTextInputPanelNode: ChatInputPanelNode, ASEditableTextNodeDelegate, ChatInputTextNodeDelegate {
    ...
    private var attachmentButtonGlowView: UIView?
    ...
    public init(context: AccountContext, presentationInterfaceState: ChatPresentationInterfaceState, presentationContext: ChatPresentationContext?, presentController: @escaping (ViewController) -> Void) {
        ...
        self.attachmentButton.highligthedChanged = { [weak self] highlighted in
            ...
            if highlighted {
                ...
                let target = self.attachmentButtonBackground
                
                let impactFeedback = UIImpactFeedbackGenerator(style: .medium)
                impactFeedback.impactOccurred()
                
                if self.attachmentButtonGlowView == nil {
                    ...
                    self.attachmentButtonGlowView = glow
                }
                ...
                UIView.animate(
                    ...
                    animations: {
                        ...
                        self.attachmentButtonGlowView?.backgroundColor = UIColor.white.withAlphaComponent(0.45)
                    },
                    completion: nil
                )
                
                self.attachmentButtonIcon.layer.removeAnimation(forKey: "opacity")
                self.attachmentButtonIcon.alpha = 0.7
                ...
                UIView.animateKeyframes(
                    ...
                    animations: {
                        UIView.addKeyframe(withRelativeStartTime: 0.0, relativeDuration: 0.35) {
                            ...
                            self.attachmentButtonGlowView?.backgroundColor = UIColor.white.withAlphaComponent(0.70)
                        }
                        
                        UIView.addKeyframe(withRelativeStartTime: 0.35, relativeDuration: 0.65) {
                            target.transform = .identity
                            self.attachmentButtonGlowView?.backgroundColor = UIColor.white.withAlphaComponent(0.0)
                        }
                    },
                    completion: { [weak self] _ in
                        self?.attachmentButtonGlowView?.removeFromSuperview()
                        self?.attachmentButtonGlowView = nil
                    }
                )
                
                self.attachmentButtonIcon.alpha = 1.0
                self.attachmentButtonIcon.layer.animateAlpha(from: 0.7, to: 1.0, duration: 0.2)
            }
        }
