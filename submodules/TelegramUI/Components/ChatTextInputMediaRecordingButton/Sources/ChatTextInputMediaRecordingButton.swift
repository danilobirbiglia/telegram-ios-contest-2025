...
public final class ChatTextInputMediaRecordingButton: TGModernConversationInputMicButton, TGModernConversationInputMicButtonDelegate {
    ...
    private var isLiquidGlassTouchActive: Bool = false
    ...
    private func findAnimationTarget() -> UIView? {
        
        var current: UIView? = self.superview
        while let view = current {
            if String(describing: type(of: view)).contains("GlassBackgroundView") {
                return view
            }
            let typeName = String(describing: type(of: view))
            if typeName.contains("Glass") || typeName.contains("Background") {
                return view
            }
            current = view.superview
        }
        
        return self.superview?.superview ?? self.superview
    }
    
    public override func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        let result = super.beginTracking(touch, with: event)
        
        if result && !self.isLiquidGlassTouchActive {
            self.isLiquidGlassTouchActive = true
            
            self.onTouchDown?()
            
            self.performLiquidGlassTouchDown()
        }
    ...
    public override func endTracking(_ touch: UITouch?, with event: UIEvent?) {
        ...
        if self.isLiquidGlassTouchActive {
            self.isLiquidGlassTouchActive = false
            
            self.onTouchUp?()
            
            self.performLiquidGlassTouchUp()
        }
    }
    ...
    private func performLiquidGlassTouchDown() {
        guard let target = findAnimationTarget() else {
            return
        }
        ...
        if glowView == nil {
            let glow = UIView(frame: target.bounds)
            glow.backgroundColor = UIColor.white.withAlphaComponent(0.0)
            glow.layer.cornerRadius = target.bounds.height / 2
            glow.isUserInteractionEnabled = false
            target.insertSubview(glow, at: 0)
            glowView = glow
        }
        
        UIView.animate(withDuration: 0.1) {
            self.glowView?.backgroundColor = UIColor.white.withAlphaComponent(0.45)
        }
        ...
        UIView.animate(
            ...
            animations: {
                target.transform = CGAffineTransform(scaleX: 0.85, y: 0.85)
            },
            completion: nil
        )
    ...
    private func performLiquidGlassTouchUp() {
        guard let target = findAnimationTarget() else { return }
        ...
        UIView.animateKeyframes(
            ...
            animations: {
                UIView.addKeyframe(withRelativeStartTime: 0, relativeDuration: 0.35) {
                    target.transform = CGAffineTransform(scaleX: 1.25, y: 1.25)
                    self.glowView?.backgroundColor = UIColor.white.withAlphaComponent(0.7)
                }
                UIView.addKeyframe(withRelativeStartTime: 0.35, relativeDuration: 0.65) {
                    target.transform = .identity
                    self.glowView?.backgroundColor = UIColor.white.withAlphaComponent(0.0)
                }
            },
            completion: { _ in
